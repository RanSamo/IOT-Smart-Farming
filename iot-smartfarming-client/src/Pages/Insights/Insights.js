...
const cardData = [
  {
    ...
    text: "Get insights based on today's data",
  },
  {
    icon: <Timeline sx={{ fontSize: "2rem", color: "#f87171" }} />,
    text: "Learn more about your farm based on all-time data",
  },
  {
    icon: <Agriculture sx={{ fontSize: "2rem", color: "#a78bfa" }} />,
    text: "Farming tips for your farm",
  },
];
...
const Insights = () => {
  ...
  const handleButtonClick = async (buttonNumber) => {
    ...
    const lastData = await response.json();
    console.log(lastData.lastData);
    const lastDataString =
      buttonNumber == 0
        ? JSON.stringify(lastData.lastData)
        : buttonNumber == 1
        ? JSON.stringify(lastData)
        : "";
    if (lastDataString == "") {
      aiText = await getAiInsights(
        `I have an outdoor farm, can you give me 3-4 points of general tips ?`
      );
    } else {
      aiText = await getAiInsights(
        `How can I improve my farm? provide the data in bullet points, keep it short , like 3-4 points. this is my data :  ${lastDataString}`
      );
    }
    setAiResponse(aiText);
  };
