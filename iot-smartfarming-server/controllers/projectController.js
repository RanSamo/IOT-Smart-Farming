...
const getSingleData = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send(`No data with id: ${id}`); // catch invalid id.

  const singleData = await projdata.findById(id);
  if (!singleData)
    return res.status(404).json({ message: `No data with id: ${id}` });
...
const getLastData = async (req, res) => {
  ...
  try {
    ...
    if (!lastData)
      return res
        .status(404)
        .json({ message: `No data found for farmId: ${farmId}` });
...
const createData = async (req, res) => {
  try {
    ...
    const farmIds = await mongoose.connection
      .collection("farms")
      .distinct("_id");

    // Function to generate random sensor data
    const generateTemperature = () =>
      (Math.random() * (30 - 20) + 20).toFixed(2); // 20 to 30 degrees Celsius.
    const generateHumidity = () => (Math.random() * (70 - 50) + 50).toFixed(2); // 50% to 70%.
    const generateSoilMoisture = () =>
      (Math.random() * (60 - 20) + 20).toFixed(2); // 20% to 60%.
...
const updateData = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send(`No data with id: ${id}`); // catch invalid id.
  const data = await projdata.findOneAndUpdate(
    { _id: id },
    {
